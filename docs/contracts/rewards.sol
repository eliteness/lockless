 ...
 * Based on FTM.Guru, Curve.fi, Synthetix.io, Convex Yearn.finance.
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
contract GuruMultiRewardFarmland is ReentrancyGuard, Pausable {
    ...
    // @notice Stake for someone else / Gift
    function stake(uint256 amount, address _ben) external nonReentrant notPaused updateReward(msg.sender) {
        _stake(amount,_ben);
    }
    ...
    // Gift someone a specific set of rewards, in one go.
    function getReward(address[] calldata _rewardsTokens, address _ben) external nonReentrant updateReward(msg.sender) {
        ...
        for (uint i; i < _rewardsTokens.length; i++) {
            ...
            if (_reward > 0) {
                rewards[msg.sender][_rewardsToken] = 0;
                IERC20(_rewardsToken).safeTransfer(_ben, _reward);
                emit RewardPaid(msg.sender, _rewardsToken, _reward);
            }
        }
    }
